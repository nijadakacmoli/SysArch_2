...
case class StateUpdate(
    ...
    def merge(update: StateUpdate): StateUpdate = {
        ...
        val new_display_output = new ValueUpdate(this.display_output.oldValue, if (update.display_output.newValue == this.display_output.oldValue) this.display_output.newValue else update.display_output.newValue)
